    ...
    while (getline(keywords_file, line)) {
        ...
        for (int i = 0; i < line.length() - 1; i++) {
            if (dfa_keyword[state - 1][line[i] - 97] == 0) {
                ...
                state = dfa_keyword[state - 1][line[i] - 97];
        }
        dfa_keyword[state - 1][line[line.length() - 1] - 97] = 25;
    }
    ofstream f;
    f.open("./output/DFA_for_Lexical.txt");
    
    f << "DFA created for keywords (Transition Table (25 X 26)):\n\n";
    for (int i = 0; i < 25; i++) {
        for (int j = 0; j < 26; j++) 
        {
            f << dfa_keyword[i][j] << " ";
...
bool is_id(const string& str) {
    int dfa_id[3][3] = {{2, 0, 0}, {2, 2, 0}, {0, 0, 0}};
    int state = 1, final_state = 2;
    for (char c : str) {
        if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || c == '_')
            state = dfa_id[state - 1][0];
        else if (c >= '0' && c <= '9')
            state = dfa_id[state - 1][1];
        else
            state = dfa_id[state - 1][2];
        if (state == 0) break;
    }
...
int is_literal(const string& str) {
    int dfa_literal[7][3] = {{2, 7, 0}, {4, 0, 3}, {4, 0, 5}, {0, 0, 0}, {6, 0, 5}, {0, 0, 0}, {7, 6, 7}};
    int state = 1, final_char = 4, final_string = 6;
    for (char c : str) {
        if (c == '\'')
            state = dfa_literal[state - 1][0];
        else if (c == '\"')
            state = dfa_literal[state - 1][1];
        else
            state = dfa_literal[state - 1][2];
        if (state == 0) break;
    }
...
int is_integer(const string& str) {
    int dfa_int_float[7][5] = {{1, 0, 0, 0, 6}, {1, 1, 2, 1, 6}, {2, 3, 2, 2, 6}, {4, 3, 3, 5, 6}, {4, 4, 4, 4, 6}, {4, 5, 5, 5, 6}, {6, 6, 6, 6, 6}};
    int final_int[] = {1, 4};
    int final_float = 2;
    int state = 0;
    for (char c : str) {
        if (c >= '0' && c <= '9')
            state = dfa_int_float[state][0];
        else if (c == 'e' || c == 'E')
            state = dfa_int_float[state][1];
        else if (c == '.')
            state = dfa_int_float[state][2];
        else if (c == '+' || c == '-')
            state = dfa_int_float[state][3];
        else
            state = dfa_int_float[state][4];
    }
...
bool is_singleline_comment(const string& str) {
    int dfa_comment[3][2] = {{2, 0}, {3, 0}, {3, 3}};
...
bool is_multiline_comment_start(const string& str) {
    int dfa_comment[3][3] = {{2, 0, 0}, {0, 3, 0}, {3, 3, 3}};
    int state = 1, final_state = 3;
    for (char c : str) {
        if (c == '/')
            state = dfa_comment[state - 1][0];
...
bool is_multiline_comment_end(const string& str) {
    int dfa_comment[3][3] = {{2, 0, 1}, {0, 3, 0}, {3, 3, 3}};
    int state = 1, final_state = 3;
    for (char c : str) {
        if (c == '*')
            state = dfa_comment[state - 1][0];
...
void parse_input_file(ifstream& input_file) {
    ...
    while (getline(input_file, line)) {
        ...
        for (int loop_ind = 0; loop_ind < line.length(); ++loop_ind) {
            ...
            const char next_token_char = loop_ind == line.length() - 1 ? '\n' : line[loop_ind + 1];
            const char prev_token_char = loop_ind == 0 ? '\n' : line[loop_ind - 1];
            bool char_is_ops = false;
            int char_op_type = -1;
            // check if it is a space or an character operator
            for (const auto& operator_bundle : char_token_types) {
                if (token_char == ' ') break;
                if (token_char == '/' && (next_token_char == '/' || next_token_char == '*' || prev_token_char == '/' || prev_token_char == '*'))
                    break;
                if (token_char == '*' && (next_token_char == '/' || prev_token_char == '/')) break;
                if (find(operator_bundle.second.second.begin(), operator_bundle.second.second.end(), token_char) !=
            ...
            if (char_is_ops || token_char == ' ') {
                ...
                // we check the type of token and process it
                    ...
                    tokens.push_back(make_tuple(token_id_map[token], token, CHAR_ID, lineNo, start));
                    ...
                    if (is_integer(token) == 1)
                        tokens.push_back(make_tuple(token_id_map[token], token, INTEGER_ID, lineNo, start));
                    else if (is_integer(token) == 2)
                        tokens.push_back(make_tuple(token_id_map[token], token, FLOAT_ID, lineNo, start));
                    ...
                    if (is_id(token))
                        ...
                        tokens.push_back(make_tuple(token_id_map[token], token, INVALID_IDENTIFIER_ID, lineNo, start));
                }
                if (token_char == ' ')
        ...
        if (token != "" && !singleline_comment) {
            if (multiline_comment) {
                ...
                if (is_integer(token) == 1)
                    tokens.push_back(make_tuple(token_id_map.size() - 1, token, INTEGER_ID, lineNo, start));
                else if (is_integer(token) == 2)
                    tokens.push_back(make_tuple(token_id_map[token], token, FLOAT_ID, lineNo, start));
            }
...
string getTypeString(int typeId) {
    switch (typeId) {
        ...
        case 99:
            return "invalid-id";
        default:
            return (*(char_token_types.find(typeId))).second.first;
    }
    return "";
}
...
void write_output_file(ofstream& fout) {
    fout << setw(10) << "id_no" << setw(20) << "lexeme" << setw(25) << "type" << setw(10) << "line_no" << setw(10) << "char_no" << '\n';
    for (int i = 0; i < 75; i++) fout << '-';
    fout << '\n';
    for (const auto& t : tokens) {
        fout << setw(10) << get<0>(t) << setw(20) << get<1>(t) << setw(25) << getTypeString(get<2>(t)) << setw(10) << get<3>(t) << setw(10)
             << get<4>(t) << "\n";
    }
}
